...
const SECOND = 1 * TENTH_OF_SECOND;
...
const BANNED_PLAYERS = ["372d9d544f07023b7637fc7e", "ead940199c7d9717e5149919", "7f1da3667024f8395adf6a12", "842bee219fe147b595cec905"]; // empty for now
const LIMITED_PLAYERS = ["8c81505ab941e0760697d777", "ead940199c7d9717e5149919"];
...
const ALLOW_ALL_INTRUMENTS = true; // removes percussion instruments (turning this on makes a lot of MIDIs sound bad)
...
const BOT_DESCRIPTION = DESCRIPTION + " Made with node.js (kinda)         lol"
const BASE_COMMANDS = [
    ["𝓱𝓮𝓵𝓹 (with that you can do the 'say' command)", "displays info about command, but no command entered shows the commands"],
    ["𝓯𝓮𝓮𝓭𝓫𝓪𝓬𝓴", "shows link to send feedback about the bot to the developer"],
    ["𝓹𝓲𝓷𝓰", "gets the milliseconds response time"]
];
const BOT_COMMANDS = [
    ["𝓹𝓵𝓪𝔂 [MIDI URL]", "plays a specific song (URL must be a direct link to a MIDI file)"],
    ["𝓼𝓽𝓸𝓹", "stops all music from playing"],
    ["𝓹𝓪𝓾𝓼𝓮", "pauses the music at that moment in the song"],
    ["𝓻𝓮𝓼𝓾𝓶𝓮", "plays music right where pause left off"],
    ["𝓼𝓸𝓷𝓰", "shows the current song playing and at what moment in time"],
    ["𝓻𝓮𝓹𝓮𝓪𝓽", "toggles repeating current song on or off"],
    ["𝓼𝓾𝓼𝓽𝓪𝓲𝓷", "toggles how sustain is controlled via either MIDI or by MPP"]
];
const BOT_OWNER_COMMANDS = [
    [BOT_ACTIVATOR, "toggles the public bot commands on or off"]
];
const PRE_MSG = NAME + "";
const PRE_HELP = PRE_MSG + "";
const PRE_FEEDBACK = PRE_MSG + "[𝓯𝓮𝓮𝓭𝓫𝓪𝓬𝓴]";
const PRE_PING = PRE_MSG + "[𝓹𝓲𝓷𝓰]";
const PRE_PLAY = PRE_MSG + "[𝓹𝓵𝓪𝔂]";
const PRE_STOP = PRE_MSG + "[𝓼𝓽𝓸𝓹]";
const PRE_PAUSE = PRE_MSG + "[𝓹𝓪𝓾𝓼𝓮]";
const PRE_RESUME = PRE_MSG + "[𝓻𝓮𝓼𝓾𝓶𝓮]";
const PRE_SONG = PRE_MSG + "[𝓼𝓸𝓷𝓰]";
const PRE_REPEAT = PRE_MSG + "[𝓻𝓮𝓹𝓮𝓪𝓽]";
const PRE_SUSTAIN = PRE_MSG + "[𝓼𝓾𝓼𝓽𝓪𝓲𝓷]";
const PRE_ABOUTME = PRE_MSG + "[𝓪𝓫𝓸𝓾𝓽 𝓶𝓮]";
const PRE_DOWNLOADING = PRE_MSG + "[𝓭𝓸𝔀𝓷𝓵𝓸𝓪𝓭𝓲𝓷𝓰]";
const PRE_LOAD_MUSIC = PRE_MSG + "[𝓵𝓸𝓪𝓭 𝓶𝓾𝓼𝓲𝓬]";
const PRE_PUBLIC = PRE_MSG + "[𝓹𝓾𝓫𝓵𝓲𝓬]";
const PRE_LIMITED = PRE_MSG + "";
const PRE_ERROR = PRE_MSG + "e r r o r...";
const WHERE_TO_FIND_MIDIS = ". ";
const NOT_OWNER = "The bot isn't the owner of the room";
const NO_SONG = "i am not playing";
const LIST_BULLET = " >   < ";
...
var publicOption = true; // turn off the public bot commands if needed
var pinging = false; // helps aid in getting response time
var pingTime = 1; // changes after each ping
...
var humanMusic = function() {
    ...
    setTimeout(function() {
        if (preventsLoadingMusic()) MPP.press("e5", 1);
        if (preventsLoadingMusic()) MPP.release("e5");
    }, 700);
    setTimeout(function() {
        if (preventsLoadingMusic()) MPP.press("c5", 1);
        if (preventsLoadingMusic()) MPP.release("c5");
    ...
    try {
        ...
        var showSongName = setInterval(function() {
            if (Player.isPlaying()) {
                ...
                console.log(' ' + getSongTimesFormatted(currentSongElapsedFormatted, currentSongDurationFormatted) + " 𝓷𝓸𝔀 𝓹𝓵𝓪𝔂𝓲𝓷𝓰 " + quoteString(currentSongName));
        ...
        console.log(PRE_ERROR + " [𝓹𝓵𝓪𝔂] " + error);
    }
}
...
var playerLimited = function(username) {
    ...
    mppChatSend(PRE_LIMITED + " ✧･ﾟ: *✧･ﾟ:* 　　 *:･ﾟ✧*:･ﾟ✧   ✧･ﾟ: *✧･ﾟ:* 　　 *:･ﾟ✧*:･ﾟ✧");
    mppChatSend(" ✧･ﾟ: *✧･ﾟ:* 　　 *:･ﾟ✧*:･ﾟ✧  ✧･ﾟ: *✧･ﾟ:* 　　 *:･ﾟ✧*:･ﾟ✧ ")
    mppChatSend("no " + (username));
}

// When there is an incorrect command, show this error
var cmdNotFound = function(cmd) {
    var error = mppChatSend (cmd) + console.log(" doesn't exist");
...
var help = function(command, userId, yourId) {
    ...
    if (!exists(command) || command == "") {
        ...
        mppChatSend(" Hi! My commands are: " + formattedCommands(BASE_COMMANDS, LIST_BULLET + PREFIX, true)
        ...
        if (exists(valid)) mppChatSend(formatCommandInfo(commandArray, commandIndex),);
        else cmdNotFound(command);
    }
}

var feedback = function() {
    mppChatSend(PRE_FEEDBACK + " Please go to hell") + (FEEDBACK_URL + " in order to submit feedback.");
}
var ping = function() {
    ...
    pingTime = Date.now();
    setTimeout(function() {
        if (pinging) mppChatSend("don't be impressed if the ping is that high..") + mppChatSend(pingTime - 1605891710060 + " ms");
        pinging = false;
    }, TENTH_OF_SECOND);
}
var play = function(url) {
    ...
    if (!exists(url) || url == "") {
        stopLoadingMusic();
        mppChatSend(" no midi url no song. " + WHERE_TO_FIND_MIDIS);
    } else {
        // downloads file if possible and then plays it if it's a MIDI
        urlToBlob(url, function(blob) {
            if (blob == null) mppChatSend(" I can't read the midi file.. if you already tried from bitmidi.com or midiworld.com, try another song. " + quoteString(url) + "... " + WHERE_TO_FIND_MIDIS);
            ...
            } else mppChatSend(error + " i caught an error... if you already tried from bitmidi.com or midiworld.com, try another song." + WHERE_TO_FIND_MIDIS);
        });
    }
}
var stop = function() {
    ...
    if (ended) mppChatSend(PRE_STOP + ' ' + NO_SONG);
        ...
        stopSong();
        paused = false;
        console.log(PRE_STOP + " 𝓼𝓽𝓸𝓹𝓹𝓮𝓭 𝓹𝓵𝓪𝔂𝓲𝓷𝓰 " + quoteString(currentSongName));
...
var pause = function() {
    ...
    if (ended) mppChatSend(PRE_PAUSE + ' ' + NO_SONG);
        ...
        if (paused) console.log(title + " The song is already paused");
        else {
            Player.pause();
            paused = true;
            console.log(title + " 𝓹𝓪𝓾𝓼𝓮𝓭 " + quoteString(currentSongName));
        }
    }
}
var resume = function() {
    ...
    if (ended) mppChatSend(PRE_RESUME + ' ' + NO_SONG);
        ...
        if (paused) {
            Player.play();
            paused = false;
            console.log(title + " 𝓻𝓮𝓼𝓾𝓶𝓮𝓭 " + quoteString(currentSongName));
        } else console.log(title + " The song is already playing");
    }
}
var song = function() {
    // shows current song playing
    if (exists(currentSongName) && currentSongName != "") {
        mppChatSend(' ' + getSongTimesFormatted(currentSongElapsedFormatted, currentSongDurationFormatted)
                                   + " Currently " + (paused ? "paused on" : "playing") + ' ' + (currentSongName));
...
var repeat = function() {
    ...
    console.log(PRE_REPEAT + " Repeat set to " + (repeatOption ? "" : "not") + " repeating");
}
var sustain = function() {
    // turns on or off sustain
    sustainOption = !sustainOption;

    console.log(PRE_SUSTAIN + " Sustain set to " + (sustainOption ? "MIDI controlled" : "MPP controlled"));
}
var loading = function(userId, yourId) {
    ...
    console.log(PRE_LOAD_MUSIC + " The MIDI loading progress is now set to " + (loadingOption ? "audio" : "text"));
}
var public = function(userId, yourId) {
    // only let the bot owner set if public bot commands should be on or not
    if (userId != yourId) return;
    publicOption = !publicOption;
    console.log(PRE_PUBLIC + " Public bot commands were turned " + (publicOption ? "on" : "off"));
}
